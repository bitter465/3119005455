...
if __name__ == '__main__':
    ...
    if model_name == "drcn": # 如果是drcn 就额外需要词语生成的词向量模型
        ...
        p, h, y = load_char_data('input/train.csv', data_size=None,maxlen=params['input_shapes'][0][0]) # 加载训练集数据
        x = [p,h]
        y = keras.utils.to_categorical(y,num_classes=params['num_classes']) # 把标签转换成独热码
        p_eval, h_eval, y_eval = load_char_data('input/dev.csv', data_size=None,maxlen=params['input_shapes'][0][0]) # 验证集
        x_eval = [p_eval,h_eval]
        y_eval = keras.utils.to_categorical(y_eval,num_classes=params['num_classes'])
        p_test, h_test, y_test = load_char_data('input/test.csv', data_size=None,maxlen=params['input_shapes'][0][0]) # 测试集
    ...
    model.compile( # 进行编译
        ...
        optimizer='adam', # 优化器
        metrics=['accuracy'] # 准确率作为评价指标
        )
    print(model.summary()) # 打印模型结构

    earlystop = keras.callbacks.EarlyStopping( # 早停
    ...
    checkpoint = keras.callbacks.ModelCheckpoint( # 保存最优
        bast_model_filepath, 
        monitor='val_accuracy', 
        verbose=1, 
        save_best_only=True,
    ...
    with open(model_frame_path, "w") as json_file: # 保存模型网络结构
    ...
    # model = keras.models.model_from_json( # 加载网络模型的json格式
